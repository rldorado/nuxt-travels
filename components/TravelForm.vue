...
import { convertDateToInputFormat } from '~/utils/formatters'
...
const submitForm = () => {
  if (!props.initialTravel) {
     // INFO: We use 'id' as a unique identifier
    travel.value.id = Math.floor(Math.random() * 10000);
  }
...
watchEffect(() => {
  if (props.initialTravel) {
    travel.value = {
      ...props.initialTravel,
      departureDate: props.initialTravel.departureDate ? convertDateToInputFormat(props.initialTravel.departureDate) : '',
      returnDate: props.initialTravel.returnDate ? convertDateToInputFormat(props.initialTravel.returnDate) : ''
    };
  } else {
    travel.value = { ...DEFAULT_EMPTY_TRAVEL };
  }
